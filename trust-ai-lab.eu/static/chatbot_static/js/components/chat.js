...
window.addEventListener('message', function(event) {
  if (event.data.type === 'initData') {
    ...
    latestExperimentLog = event.data.experimentData;
  }
});
/**
...
window.addEventListener('message', function(event) {
    if (event.data.type === 'sendReadyMessage') {
        const message = event.data.message;
        setUserResponse(message); // Display "ready" in the chat window
        send(message); // Send the "ready" message to Rasa
    }
});
...
window.addEventListener('message', function(event) {
    if (event.data.type === 'sendMessage') {
        const message = event.data.message;
        setUserResponse(message);  // Display the message in the chat
        send(message);             // Send the message to Rasa
    }
});
